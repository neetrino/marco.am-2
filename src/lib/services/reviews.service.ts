...
class ReviewsService {
  ...
  async getProductReviews(productId: string, options?: { publishedOnly?: boolean }) {
    ...
    logger.devLog(`✅ [REVIEWS SERVICE] Found ${reviews.length} reviews for product ${productId}`);
  ...
  async getUserReview(productId: string, userId: string, includeUnpublished: boolean = true) {
    ...
    logger.devLog('📝 [REVIEWS SERVICE] Getting user review:', { productId, userId });
    ...
    if (!review) {
      logger.devLog('📝 [REVIEWS SERVICE] No review found for user');
      return null;
    }

    // If includeUnpublished is false, only return published reviews
    if (!includeUnpublished && !review.published) {
      logger.devLog('📝 [REVIEWS SERVICE] Review exists but is not published');
      return null;
    }

    logger.devLog('✅ [REVIEWS SERVICE] Found user review:', review.id);
  ...
  async createReview(
    ...
    logger.devLog('✅ [REVIEWS SERVICE] Review created:', review.id);
