...
class AdminSettingsService {
  ...
  async updateSettings(data: any) {
    ...
    if (data.defaultCurrency !== undefined) {
      ...
      logger.devLog('✅ [ADMIN SERVICE] Default currency updated:', currencyValue);
    }
  ...
  async getPriceFilterSettings() {
    logger.devLog('⚙️ [ADMIN SERVICE] Fetching price filter settings...');
    const setting = await db.settings.findUnique({
      where: { key: 'price-filter' },
    });

    if (!setting) {
      logger.devLog('✅ [ADMIN SERVICE] Price filter settings not found, returning defaults');
      return {
        minPrice: null,
        maxPrice: null,
        stepSize: null,
    ...
    logger.devLog('✅ [ADMIN SERVICE] Price filter settings loaded:', value);
    return {
  ...
  async updatePriceFilterSettings(data: {
    ...
    logger.devLog('⚙️ [ADMIN SERVICE] Updating price filter settings...', data);
