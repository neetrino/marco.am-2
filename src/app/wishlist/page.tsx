...
export default function WishlistPage() {
  ...
  const fetchWishlistProducts = useCallback(async (idsToLoad: string[]) => {
    if (idsToLoad.length === 0) {
      logger.info('[Wishlist] Skip fetch because ids array is empty');
      setProducts([]);
      setLoading(false);
      return;
    }
    ...
    try {
      setLoading(true);
      logger.info(`[Wishlist] Fetching ${idsToLoad.length} products for render`);
  ...
  const handleRemove = (productId: string) => {
    logger.info(`[Wishlist] Removing product ${productId} from wishlist UI`);
