...
export function useVariantConversionToFormData({
  ...
  const convertGeneratedVariantsToFormData = (): void => {
    ...
    if (productType === 'variable' && selectedAttributesForVariants.size > 0 && generatedVariants.length > 0) {
      logger.devLog('🔄 [ADMIN] Converting new variant builder variants to formData format...');
      logger.devLog('📊 [ADMIN] Total generated variants to convert:', generatedVariants.length);
          ...
          selectedColorValueIds.forEach((valueId) => {
            ...
            if (value) {
              ...
              if (value.imageUrl) {
                ...
                logger.devLog('✅ [ADMIN] Added attribute value imageUrl to color:', value.imageUrl);
              }
          ...
          selectedSizeValueIds.forEach((sizeValueId) => {
            ...
            if (sizeValue) {
              ...
              if (sizeValue.imageUrl) {
                ...
                logger.devLog('✅ [ADMIN] Added size attribute value imageUrl:', sizeValue.imageUrl);
              }
            }
          });
            ...
            otherAttributeIds.forEach((attributeId) => {
              ...
              selectedValueIds.forEach((valueId) => {
                ...
                if (value && value.imageUrl) {
                  colorData.images.push(value.imageUrl);
                  logger.devLog('✅ [ADMIN] Added attribute value imageUrl from', attribute.key, ':', value.imageUrl);
                }
              });
            });
      ...
      logger.devLog('✅ [ADMIN] Converted variants:', allNewVariants.length, 'with unique SKUs');
    }
  };
