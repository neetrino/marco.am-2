...
export function useProductAttributeHandlers({
  ...
  const handleAddColor = async () => {
    ...
    try {
      setAddingColor(true);
      const response = await apiClient.post<{ data: Attribute }>(`/api/v1/supersudo/attributes/${colorAttribute.id}/values`, {
      ...
      setColorMessage({ type: 'error', text: getApiOrErrorMessage(err, t('admin.products.add.failedToAddColor')) });
    } finally {
  ...
  const handleAddSize = async () => {
    ...
    try {
      setAddingSize(true);
      const response = await apiClient.post<{ data: Attribute }>(`/api/v1/supersudo/attributes/${sizeAttribute.id}/values`, {
      ...
      setSizeMessage({ type: 'error', text: getApiOrErrorMessage(err, t('admin.products.add.failedToAddSize')) });
    } finally {
      setAddingSize(false);
    }
  };
