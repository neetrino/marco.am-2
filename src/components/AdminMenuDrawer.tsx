...
export function AdminMenuDrawer({ tabs, currentPath }: AdminMenuDrawerProps) {
  ...
  useEffect(() => {
    if (open) {
      logger.info('[AdminMenuDrawer] Locking body scroll for open drawer');
  ...
  const handleNavigate = (path: string) => {
    logger.info('[AdminMenuDrawer] Navigating to admin path', { path });
    router.push(path);
    setOpen(false);
  };
  ...
  return (
    <div className="lg:hidden">
      <button
        type="button"
        onClick={() => {
          logger.info('[AdminMenuDrawer] Toggling drawer', { open: !open });
          setOpen(true);
        }}
      ...
      {open && (
        <div
          ...
          onClick={() => {
            logger.info('[AdminMenuDrawer] Closing drawer from backdrop');
            setOpen(false);
          }}
        >
          <div
          ...
          >
            <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
              ...
              <button
                type="button"
                onClick={() => {
                  logger.info('[AdminMenuDrawer] Closing drawer from close button');
                  setOpen(false);
                }}
