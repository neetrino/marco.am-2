...
export function ProfileMenuDrawer({ tabs, activeTab, onSelect }: ProfileMenuDrawerProps) {
  ...
  useEffect(() => {
    if (open) {
      logger.devInfo('[ProfileMenuDrawer] Locking body scroll');
  ...
  const handleSelect = (tabId: string) => {
    logger.devInfo('[ProfileMenuDrawer] Selecting tab', { tabId });
    onSelect(tabId);
    setOpen(false);
  };
    ...
    <div className="lg:hidden">
      ...
      {open && (
        <div
          ...
          onClick={() => {
            logger.devInfo('[ProfileMenuDrawer] Closing drawer from backdrop');
            setOpen(false);
          }}
        >
          <div
